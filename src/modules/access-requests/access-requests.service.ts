...
export class AccessRequestsService {
  ...
  async createAccessRequest(dto: CreateAccessRequestDto, requester: User): Promise<AccessRequest> {
    ...
    const node = await this.nodeService.findNodeById(targetNodeId);
    if (!node) {
      throw new BusinessException(ErrorCode.DOCUMENT_NOT_FOUND, ErrorMessages.DOCUMENT_NOT_FOUND, 404);
    }
    // 1b. Kiểm tra xem người dùng đã có quyền cao hơn hoặc bằng quyền đang xin chưa
    const hasSufficentPermission = await this.permissionService.checkUserPermissionForNode(requester, targetNodeId, requestedPermission);
    if (hasSufficentPermission) {
      throw new BusinessException(ErrorCode.ACCESS_DENIED, ErrorMessages.ALREADY_HAS_PERMISSION, 409);
    }
    ...
    if (existingPendingRequest) {
      throw new BusinessException(ErrorCode.ACCESS_DENIED, ErrorMessages.PENDING_REQUEST_EXISTS, 409);
    }
    ...
    try {
      ...
      const owners = await this.usersService.findUsersByIds(ownerIds);
  ...
  private async findRequestAndCheckReviewerPermission(requestId: string, reviewer: User): Promise<AccessRequest> {
    ...
    if (!request) throw new BusinessException(ErrorCode.REQUEST_NOT_FOUND, ErrorMessages.REQUEST_NOT_FOUND, 404);
    if (request.status !== RequestStatus.PENDING) {
      throw new BusinessException(ErrorCode.BAD_REQUEST, ErrorMessages.REQUEST_ALREADY_PROCESSED, 400);
    }

    const canReview = await this.permissionService.checkUserPermissionForNode(reviewer, request.nodeId, PermissionLevel.OWNER);
    if (!canReview) throw new BusinessException(ErrorCode.ACCESS_DENIED, ErrorMessages.ACCESS_DENIED, 403);
  ...
  private async sendResultNotification(request: AccessRequest) {
    try {
      const [requester, node] = await Promise.all([
        this.usersService.findUserById(request.requesterId.toHexString()),
        this.nodeService.findNodeById(request.nodeId),
      ]);
