...
describe("PermissionsService", () => {
  ...
  it("should return true for RootAdmin regardless of permissions", async () => {
    const result = await service.checkUserPermissionForNode(mockRootAdmin, new ObjectId(), PermissionLevel.OWNER);
  ...
  it("should return false if no permission record is found for a regular user", async () => {
    ...
    const result = await service.checkUserPermissionForNode(mockUser, new ObjectId(), PermissionLevel.VIEWER);
  ...
  it("should return true if user has the exact required permission", async () => {
    ...
    const result = await service.checkUserPermissionForNode(mockUser, new ObjectId(), PermissionLevel.EDITOR);
    expect(result).toBe(true);
  });
